...
static gboolean emit_delete_event_signal (XtmProcessWindow *window);
...
static gboolean
emit_delete_event_signal (XtmProcessWindow *window)
{
	xtm_process_window_store_size (window);
	gboolean ret = FALSE;
	g_signal_emit_by_name (window, "delete-event", NULL, &ret, G_TYPE_BOOLEAN);
	return ret;
}
