...
build_context_menu (XtmProcessTreeView *treeview, GPid pid)
{
	...
	for (guint8 i = 0; i != N; ++i)
	{
		...
		else
		{
			...
			if (i == COPY_COMMAND_LINE)
			{
				...
				gchar *caption;
				caption = _(CAPTIONS[i]);
				accel_group = gtk_accel_group_new ();
				label = gtk_accel_label_new (caption);
				...
				gtk_window_add_accel_group (GTK_WINDOW (gtk_widget_get_toplevel(mi)), accel_group);
				gtk_accel_label_set_accel_widget (GTK_ACCEL_LABEL (label), mi);
				gtk_label_set_xalign (GTK_LABEL (label), 0.0);
				gtk_label_set_width_chars (GTK_LABEL (label), g_utf8_strlen (caption, -1) + 1);
			}
			else
			{
				label = gtk_label_new (_(CAPTIONS[i]));
	...
	const gchar *PRIO_CAPTIONS[] = {"Very low", "Low", "Normal", "High", "Very high"};
	const gint PRIORITIES[] = {XTM_PRIORITY_VERY_LOW, XTM_PRIORITY_LOW, XTM_PRIORITY_NORMAL, XTM_PRIORITY_HIGH, XTM_PRIORITY_VERY_HIGH};
