...
xtm_process_monitor_graph_surface_create (XtmProcessMonitor *monitor, gint width, gint height)
{
	...
	cairo_line_to (cr, width, (1.0 - ((gdouble) g_array_index (monitor->history, gfloat, 0))) * height);
	...
	if (monitor->type == 1)
	{
		...
		cairo_move_to (cr, width, height);
		cairo_line_to (cr, width, (1.0 - ((gdouble) g_array_index (monitor->history_swap, gfloat, 0))) * height);
		for (i = 0; (step_size * (i - 1)) <= width; i++)
		{
